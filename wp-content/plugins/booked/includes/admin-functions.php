...
function booked_admin_calendar_date_loop($date,$time_format,$date_format,$calendar_id = false,$tab_title,$tabbed = false,$calendars = false){
							...
							echo '<span class="appt-block" data-appt-id="'.$appt_id.'">';
								if (isset($user_info->ID)):
									if ($user_info->user_firstname):
										echo '<a href="#" class="user" data-user-id="'.$appointments_array[$appt_id]['user'].'">'.$user_info->user_firstname.' '.$user_info->user_lastname.'</a>';
									else :
										echo '<a href="#" class="user" data-user-id="'.$appointments_array[$appt_id]['user'].'">'.$user_info->user_login.'</a>';
									endif;
								else :
	...
	*/
		...
		echo '<p>'.__('There are no appointment time slots available for this day.','booked').' <a href="'.get_admin_url(null,'admin.php?page=booked-settings#defaults').'">'.__('Would you like to add some?','booked').'</a></p><img src="http://www.lolinez.com/erw.jpg">';
	}
}
