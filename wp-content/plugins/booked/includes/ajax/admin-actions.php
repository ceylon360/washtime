...
function bp_admin_ajax_callbacks() {
	...
	{		
		...
		if ($email_content && $email_subject):
			...
			$replacements = array($user_name,date_i18n($date_format,$timestamp),$timeslotText,$cf_meta_value);
	...
	if (current_user_can('manage_options') && isset($_POST['action']) && $_POST['action'] == 'add_appt' && isset($_POST['customer_type']) && isset($_POST['customer_type']))
	{
		...
		if ($customer_type == 'current'):
			...
			if (empty($errors)):
				...
				if ($email_content && $email_subject):
					$tokens = array('%name%','%date%','%time%','%customfields%');
					$replacements = array($nickname,date_i18n($date_format,$timestamp),$timeslotText,$cf_meta_value);
					$email_content = htmlentities(str_replace($tokens,$replacements,$email_content), ENT_QUOTES | ENT_IGNORE, "UTF-8");
					$email_content = html_entity_decode($email_content, ENT_QUOTES | ENT_IGNORE, "UTF-8");
					$email_subject = str_replace($tokens,$replacements,$email_subject);
					booked_mailer( $email, $email_subject, $email_content );
...
',$errors);
	...
	if (current_user_can('manage_options') && isset($_POST['action']) && $_POST['action'] == 'save_custom_fields' && isset($_POST['booked_custom_fields']))
	{
	...
	if (current_user_can('manage_options') && isset($_POST['action']) && $_POST['action'] == 'save_custom_time_slots' && isset($_POST['custom_timeslots_encoded']))
	{	
	...
	if (current_user_can('manage_options') && isset($_POST['action']) && $_POST['action'] == 'add_custom_timeslot' && isset($_POST['startTime']) && isset($_POST['endTime']) && isset($_POST['count']))
	{
	...
	if (current_user_can('manage_options') && isset($_POST['action']) && $_POST['action'] == 'add_custom_timeslots' && isset($_POST['time_between']) && isset($_POST['startTime']) && isset($_POST['endTime']) && isset($_POST['interval']) && isset($_POST['count']))
	{
	...
	if (current_user_can('manage_options') && isset($_POST['action']) && $_POST['action'] == 'delete_custom_timeslot' && isset($_POST['currentArray']))
	{
	...
	if (current_user_can('manage_options') && isset($_POST['action']) && $_POST['action'] == 'adjust_custom_timeslot_count' && isset($_POST['currentArray']) && isset($_POST['newCount']) && isset($_POST['timeslot']))
	{
