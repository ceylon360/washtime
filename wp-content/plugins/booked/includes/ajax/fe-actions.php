...
',$errors);
	...
	{		
		...
		$timeslot = get_post_meta($_GET['appt_id'],'_appointment_timeslot',true);
		$timestamp = get_post_meta($_GET['appt_id'],'_appointment_timestamp',true);
		$cf_meta_value = get_post_meta($_GET['appt_id'],'_cf_meta_value',true);
		$timeslots = explode('-',$timeslot);
		$time_format = get_option('time_format');
		$date_format = get_option('date_format');
