...
class BookedShortcodes {
    ...
    public function booked_calendar_switcher_shortcode( $attrs ){
		...
		$args = array( 
			...
			'hide_empty'	=> 0,
			'echo'			=> 0,
			'id'			=> 'booked_calendar_chooser',
			'name'			=> 'booked_calendar_chooser'
		);
		...
		$args['show_option_all'] = 'Default Calendar';
	...
	public function booked_calendar_shortcode($atts, $content = null) {
		...
		if ($atts['switcher']):
			$args = array( 
				...
				'id'			=> 'booked_calendar_chooser',
				'name'			=> 'booked_calendar_chooser',
				'show_option_all'	=> __('Default Calendar','booked'),
				'selected'		=> $atts['calendar']
			);
			echo '<div id="booked-calendarSwitcher"><p><i class="fa fa-calendar"></i>' . str_replace( "\n", '', wp_dropdown_categories( $args ) ) . '</p></div>';
		endif;
