...
class AB_SMS
{
    ...
    public function register( $username, $password, $password_repeat )
    {
        ...
        if ( $password !== $password_repeat and ! empty( $password ) ) {
            $this->errors[] = __( 'Passwords must be the same.', 'ab' );
    ...
    public function sendSms( $phone_number, $message )
    {
        if ( $this->token ) {
            ...
            if ( $data['phone'] != '' ) {
                ...
                $this->errors[] = __( 'Phone number is empty.', 'ab' );
            }
        }
    ...
    public function normalizePhoneNumber( $phone_number )
    {
        ...
        if ( $phone_number{0} == '+' ) {
            // ok.
        } else if ( strpos( $phone_number, '00' ) === 0 ) {
            ...
            if ( $response ) {
                array_walk( $response->list, function( &$item ) {
                    ...
                    switch ( $item->status ) {
                        case 1:
                        case 10:
                            $item->status = __( 'Queued', 'ab' );
                            $item->charge = '$' . $item->charge;
                            break;
                        case 2:
                        case 16:
                            $item->status = __( 'Error', 'ab' );
                            $item->charge = '';
                            break;
                        case 3:
                            $item->status = __( 'Out of credit', 'ab' );
                            $item->charge = '';
                            break;
                        case 4:
                            $item->status = __( 'Country out of service', 'ab' );
                            $item->charge = '';
                            break;
                        case 11:
                            $item->status = __( 'Sending', 'ab' );
                            $item->charge = '$' . $item->charge;
                            break;
                        case 12:
                            $item->status = __( 'Sent', 'ab' );
                            $item->charge = '$' . $item->charge;
                            break;
                        case 13:
                            $item->status = __( 'Delivered', 'ab' );
                            $item->charge = '$' . $item->charge;
                            break;
                        case 14:
                            $item->status = __( 'Failed', 'ab' );
                            $item->charge = '$' . $item->charge;
                            break;
                        case 15:
                            $item->status = __( 'Undelivered', 'ab' );
                            $item->charge = '$' . $item->charge;
                            break;
                    }
                } );
    ...
    private function _handleResponse( $response )
    {
        ...
        if ( $response !== null && property_exists( $response, 'success' ) ) {
            ...
            $this->errors[] = __( 'Error connecting to server.', 'ab' );
        }
    ...
    {
        $error_codes = array(
            'ERROR_RECOVERY_CODE_EXPIRED'          => __( 'Recovery code expired.', 'ab' ),
            'ERROR_EMPTY_PASSWORD'                 => __( 'Empty password.', 'ab' ),
            'ERROR_INCORRECT_PASSWORD'             => __( 'Incorrect password.', 'ab' ),
            'ERROR_INCORRECT_RECOVERY_CODE'        => __( 'Incorrect recovery code.', 'ab' ),
            'ERROR_INCORRECT_USERNAME_OR_PASSWORD' => __( 'Incorrect email or password.', 'ab' ),
            'ERROR_INVALID_USERNAME'               => __( 'Invalid email.', 'ab' ),
            'ERROR_SENDING_EMAIL'                  => __( 'Error sending email.', 'ab' ),
            'ERROR_USER_NOT_FOUND'                 => __( 'User not found.', 'ab' ),
            'ERROR_USERNAME_ALREADY_EXISTS'        => __( 'Email already in use.', 'ab' ),
        );
        if ( array_key_exists( $error_code, $error_codes ) ){
            $message = $error_codes[ $error_code ];
        } else {
            // Build message from error code.
            $message = __( ucfirst( strtolower ( str_replace( '_', ' ', substr( $error_code, 6 ) ) ) ), 'ab' );
        }
