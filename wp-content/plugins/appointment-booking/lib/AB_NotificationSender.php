...
abstract class AB_NotificationSender
{
            ...
            case self::E_NEW_APPOINTMENT:
                foreach ( array( 'email', 'sms' ) as $gateway ) {
                    ...
                    if ( $to_staff->get( 'active' ) ) {
                        ...
                        self::_send( $to_staff, $codes, $staff->get( 'email' ), $staff->get( 'phone' ) );
                    }
    ...
    private static function _send( AB_Notification $notification, AB_NotificationCodes $codes, $mail_to, $phone = '')
    {
        ...
        if ( $notification->get( 'gateway' ) == 'email' ) {
            ...
            $result  = wp_mail( $mail_to, $subject, wpautop( $message ), AB_CommonUtils::getEmailHeaders() );

            // Send copy to administrators.
            if ( $notification->get( 'copy' ) ) {
                $admin_emails = AB_CommonUtils::getAdminEmails();
                if ( ! empty ( $admin_emails )) {
                    wp_mail( $admin_emails, $subject, $message, AB_CommonUtils::getEmailHeaders() );
                }
            }
