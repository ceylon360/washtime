    ...
    public function getAppointments( $start_date, $end_date = null )
    {
        ...
        $args = array(
            'SELECT
              ...
              LEFT JOIN `' . AB_Service::getTableName() . '` `service` ON `a`.`service_id` = `service`.`id`
    ...
    public function getStaffServices()
    {
        ...
        if ( $this->get( 'id' ) ) {
            $records = $this->wpdb->get_results( $this->wpdb->prepare(
                ...
                FROM `' . AB_StaffService::getTableName() . '` `ss` LEFT JOIN `' . AB_Service::getTableName() . '` `s` ON `s`.`id` = `ss`.`service_id`
                WHERE `ss`.`staff_id` = %d',
                $this->get( 'id' )
            ), ARRAY_A );
