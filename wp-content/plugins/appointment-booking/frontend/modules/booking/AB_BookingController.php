...
class AB_BookingController extends AB_Controller
{
    ...
    public function executeRenderService()
    {
        ...
        if ( $form_id ) {
            ...
            $response = array( 'status' => 'error', 'error' => __( 'Form ID error.', 'ab' ) );
        }
    ...
    public function executeRenderTime()
    {
        ...
        if ( $userData->load() ) {
            ...
            $response = array(
                ...
                'has_slots'      => ! empty ( $this->time ),
                'has_more_slots' => $availableTime->hasMoreSlots(),
                'holidays'       => $availableTime->getHolidaysForPickadate(),
            );
        } else {
    ...
    public function executeRenderComplete()
    {
        ...
        if ( $userData->load() ) {
            $success_html = nl2br( get_option( 'ab_appearance_text_info_fifth_step' ) );
            if ( get_option( 'ab_appearance_show_progress_tracker' ) == 1 ) {
                ...
                $success_html = $this->progress_tracker . $success_html;
            }
            ...
            if ( count( $availableTime->getTime() ) ) { // check, if there are available time
                ...
                foreach ( $availableTime->getTime() as $client_timestamp => $slot ) {
                    if ( $slot[ 'is_day' ] ) {
                        ...
                        $button = sprintf(
                            '<button %s data-date="%s" data-staff_id="%s" class="ab-available-hour ladda-button %s" value="%s" data-style="zoom-in" data-spinner-color="#333"><span class="ladda-label"><i class="ab-hour-icon"><span></span></i>%s</span></button>',
    ...
    public function executeCancelAppointment()
    {
        ...
        if ( $customer_appointment->loadBy( array( 'token' => $this->getParameter( 'token' ) ) ) ) {
            ...
            $count = $this->getWpdb()->get_var( $this->getWpdb()->prepare(
                'SELECT COUNT(*) FROM '.AB_CustomerAppointment::getTableName().' WHERE appointment_id = %d',
    ...
    private function _prepareInfoText( $booking_step, $userData, $preset_price = null )
    {
        ...
        if ( $booking_step === 2 ) {
            ...
            return strtr( nl2br( get_option( 'ab_appearance_text_info_second_step' ) ), $replacement );
        }
        ...
        if ( $booking_step === 3 ) {
            ...
            return strtr( nl2br( get_option( 'ab_appearance_text_info_third_step' ) ), $replacement );
        }
        ...
        if ( $booking_step === 4 ) {
            ...
            return strtr( nl2br( get_option( 'ab_appearance_text_info_coupon' ) ), $replacement );
        }
