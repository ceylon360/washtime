...
class AB_AvailableTime
{
    ...
    private function _prepareStaffData( DateTime $start_date )
    {
        ...
        $rows = $wpdb->get_results( $wpdb->prepare(
            "SELECT staff_id, price, capacity FROM `".AB_StaffService::getTableName()."` WHERE `staff_id` IN ({$this->_staffIdsStr}) AND `service_id` = %d",
        ...
        // Load holidays.
        $rows = $wpdb->get_results( "SELECT * FROM `" . AB_Holiday::getTableName() . "` WHERE `staff_id` IN ({$this->_staffIdsStr})", ARRAY_A );
        if ( is_array( $rows ) ) {
            foreach ( $rows as $row ) {
        ...
        $rows = $wpdb->get_results( "
            SELECT `item`.*, `break`.`start_time` AS `break_start`, `break`.`end_time` AS `break_end`
                FROM `".AB_StaffScheduleItem::getTableName()."` `item`
                LEFT JOIN `".AB_ScheduleItemBreak::getTableName()."` `break` ON `item`.`id` = `break`.`staff_schedule_item_id`
        ...
        if ( is_array( $rows ) ) {
            foreach ( $rows as $row ) {
                ...
                if ( substr( $row['end_date'], 11 ) == '00:00:00' ) {
                    ...
                    $row['end_date'] = '10_symbols 24:00:00';
                }
    ...
    /**
     * Get common holidays of all staff members in Pickadate format.
     *
     * @return array
     */
    public function getHolidaysForPickadate()
    {
        $result = array();
        $temp   = array();
        $count  = count( $this->staffData );

        foreach ( $this->staffData as $staff_id => $staff_data ) {
            foreach ( $staff_data['holidays'] as $holiday ) {
                list ( $year, $month, $day ) = array_map( 'intval', explode( '-', $holiday[ 'date' ] ) );
                $repeat = $holiday['repeat_event'] ? 10 : 1;
                for ( $i = 0; $i < $repeat; ++ $i ) {
                    $y = $year + $i;
                    if ( isset ( $temp[ $y ][ $month ][ $day ] ) ) {
                        ++ $temp[ $y ][ $month ][ $day ];
                    } else {
                        $temp[ $y ][ $month ][ $day ] = 1;
                    }
                    if ( $temp[ $y ][ $month ][ $day ] == $count ) {
                        // Add holiday to final result only if it is common for all staff members.
                        $result[] = array( $y, $month - 1, $day );
                    }
                }
            }
